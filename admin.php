<?php
// Legacy redirect: admin.php was renamed to officer.php
...
function admin_categorize($name){
...
function admin_default_slots($category, $storage_type = null, $kind = null){
  // mapping rules:
  // raw -> 7
  // materials -> 4
  // liquids -> 15 (default for liquid silo/primary liquid storages)
  // "separate liquid" style storages -> 5
...
// Ensure our admin tables exist: facilities and facility_structures
try{
    ...
    // allow multiple types per structure via a linking table
    ...
    // separate storage table: structure-level storage entries (e.g. liquid stockpile)
    ...
    try {
      $pdo->exec("ALTER TABLE facility_structures ADD COLUMN slots INTEGER DEFAULT 15");
    } catch (Exception $e) { }
    // add extended columns if missing: kind/unit/allowed_item/per_item_volume for richer storage definitions
    try {
      $pdo->exec("ALTER TABLE facility_storage ADD COLUMN unit VARCHAR(16)");
    } catch (Exception $e) { /* ignore if column exists */ }
    try {
      $pdo->exec("ALTER TABLE facility_storage ADD COLUMN allowed_item VARCHAR(255)");
    } catch (Exception $e) { }
    try {
      $pdo->exec("ALTER TABLE facility_storage ADD COLUMN per_item_volume INTEGER DEFAULT NULL");
    } catch (Exception $e) { }
    try {
      $pdo->exec("ALTER TABLE facility_storage ADD COLUMN kind VARCHAR(64) DEFAULT NULL");
    } catch (Exception $e) { }
    try {
      $pdo->exec("ALTER TABLE facility_storage ADD COLUMN slots INTEGER DEFAULT 15");
    } catch (Exception $e) { }
}catch(Exception $e){ /* ignore - DB connection issues will be reported via actions */ }
    ...
    try{
        ...
        if ($action === 'create_facility'){
          ...
          $notes = trim(isset($_POST['notes']) ? $_POST['notes'] : '');
            ...
            // Use portable upsert: check existence then insert or update so both SQLite and MySQL work
        ...
        if ($action === 'add_structure'){
          ...
          if (empty($types) && isset($_POST['type'])) $types = [ trim($_POST['type']) ];
            ...
            // use first type as the primary column value for compatibility
            ...
            // find the inserted structure id
            ...
            // Auto-create standard storages for inputs (rawresources/materials) used by this structure
            // Gather input names from DB recipes (if present)
            $inputNames = [];
            try{
            ...
            // Also scan merged JSON or individual db/*.json files for nodes with matching 'structure' field
            ...
            // Also gather output items from recipes and JSON (we will create storages for inputs AND outputs)
            $outputNames = [];
            try{
            ...
            // Also scan JSON files collected earlier for 'output' nodes
            foreach ($tryFiles as $jf){
              $txt = @file_get_contents($jf);
              if ($txt === false) continue;
              $data = json_decode($txt, true);
            ...
            // Merge inputs and outputs into a single set of item names
            $itemNames = $inputNames;
            foreach (array_keys($outputNames) as $o) $itemNames[trim($o)] = true;

            // For each unique input/output, if it is raw or materials, create a storage entry with capacity 32000
            foreach (array_keys($itemNames) as $iname){
              $iname = trim($iname);
              if ($iname === '') continue;
              $cat = admin_categorize($iname);
              if ($cat !== 'raw' && $cat !== 'materials') continue;
              // avoid duplicates: only create if no existing storage entry for this allowed_item
              ...
              $defSlots = admin_default_slots($cat, 'auto', $cat);
            ...
            // if this structure uses liquids in recipes, we do NOT auto-create any 'Separate Liquid' storage here.
            // Storage entries should be created explicitly via the UI or quick-add helpers which pass specific storageEntries.
            // Keep the received liquid capacity parameters (for possible later use)
          ...
          if ($action === 'add_storage_entry'){
            ...
            if (!$structure_id || $kind === '') jsonResp(['ok'=>false,'error'=>'structure_id and kind required']);
              // determine slots: use provided slots, otherwise determine default based on category/kind
              $slots = null;
              if (isset($_POST['slots'])) {
                $slots = intval($_POST['slots']);
              } else if ($capacity === 32000) {
                // infer category from allowed_item if available, otherwise from kind
                $inferredCat = '';
                if (!empty($allowed_item)) $inferredCat = admin_categorize($allowed_item);
                else $inferredCat = (stripos($kind,'liquid')!==false) ? 'liquids' : ((stripos($kind,'material')!==false) ? 'materials' : 'other');
                $slots = admin_default_slots($inferredCat, $kind, $kind);
              }
              if ($slots === null) {
        ...
        if ($action === 'delete_structure'){
          $id = intval(isset($_POST['id']) ? $_POST['id'] : 0);
            ...
            // 1) facility_storage_stock entries for storages belonging to this structure
            ...
            // 2) facility_storage rows for this structure
            dbExecute('DELETE FROM facility_storage WHERE structure_id = ?', [$id]);
            // 3) facility_structure_types linking rows
            dbExecute('DELETE FROM facility_structure_types WHERE structure_id = ?', [$id]);
            // 4) finally delete the structure row
            dbExecute('DELETE FROM facility_structures WHERE id=?', [$id]);
            jsonResp(['ok'=>true]);
        }
        if ($action === 'list_structure_options'){
          // offer distinct structure names from recipes table and from imported facility_structures
          ...
          if (file_exists($merged)){
            ...
            if (is_array($data)){
              foreach ($data as $node){
                ...
                // also check produced_by options
        ...
        if ($action === 'update_user_role'){
          // Only Officers may change roles
...
// If not POST, render admin UI
?>
<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Admin — Facility Structures</title>
  <style>
...
<body class="admin-page">
  ...
  <div class="admin-navbar">
    <div class="container">
      <div class="admin-brand"><span class="logo" aria-hidden="true"></span><span class="title">HUSS Admin Panel</span></div>
      <div class="admin-nav-actions">
        <a class="admin-btn" href="index.php">Home</a>
  ...
  <div class="admin-wrap">
    <div class="admin-hero">
      <h1>Facility Structures Admin</h1>
    ...
    <div class="admin-tabs" role="tablist" aria-label="Admin sections">
      <button class="admin-tab active" data-target="facilities" role="tab" aria-selected="true">Facilities</button>
      <button class="admin-tab" data-target="structures" role="tab" aria-selected="false">Manage Structures</button>
      <button class="admin-tab" data-target="users" role="tab" aria-selected="false">Users</button>
    </div>

    <div class="admin-content">
      <section id="section-facilities" class="admin-section active">
        <div class="panel">
          <h3>Facilities</h3>
      ...
      <section id="section-structures" class="admin-section">
        <div class="panel">
      ...
      <section id="section-users" class="admin-section">
        <div class="panel" id="users-panel">
          <h3>Users</h3>
          <div id="users-banner" class="admin-banner info" style="display:none">Only Officers may change roles — members can view but not modify roles.</div>
  ...
  <script>
    async function post(action, data={}){
      ...
      const res = await fetch('admin.php', {method:'POST', body: f});
    ...
    // Quick-add helper to create a structure and optionally add storage entries to it
    ...
    async function loadAssignedStructures(){
      ...
      if (!r.structures.length) return wrap.textContent = 'No structures assigned';
        ...
        if (s.icon){
          try{
            const path = String(s.icon).replace(/\\\\/g,'/');
            icon.style.backgroundImage = 'url("'+encodeURI(path)+'")';
          }catch(e){ }
        }
        info.appendChild(icon);
          const typesDisplay = (s.types && s.types.length) ? s.types.join(', ') : (s.type || '');
          const title = document.createElement('div');
          // compute slots info and total capacity for this structure
          let totalSlotsSum = 0; // sum of slots across auto storages
          let computedTotalCapacity = 0; // sum of capacity considering slots
          let autoCount = 0; // number of auto storages
          let perAutoSlots = null; // representative slots per auto storage (if consistent)
          if (s.storage && s.storage.length) {
            s.storage.forEach(st => {
              try{
                ...
                if (stType === 'auto' && isFinite(sn) && sn > 0){
                  autoCount += 1;
                  totalSlotsSum += sn;
                  if (perAutoSlots === null) perAutoSlots = sn;
                  else if (perAutoSlots !== sn) perAutoSlots = -1; // inconsistent
                  computedTotalCapacity += (isFinite(baseCap) ? baseCap * sn : 0);
                } else {
                  computedTotalCapacity += (isFinite(baseCap) ? baseCap : 0);
                }
              }catch(e){}
            });
          }
          // Build title: show computed total capacity and slot info. If multiple auto storages exist,
          // display per-auto slots × count and the total sum to avoid double-count confusion.
          let titleHtml = '<strong>'+s.structure_name+'</strong> — '+typesDisplay+' — total capacity: '+computedTotalCapacity;
          if (autoCount > 0){
            if (perAutoSlots > 0){
              // show representative per-auto slots (do not sum across multiple auto entries)
              titleHtml += ' — slots: ' + perAutoSlots;
            } else {
              titleHtml += ' — slots total: ' + totalSlotsSum + ' (' + autoCount + ' auto storages)';
            }
          }
          title.innerHTML = titleHtml;
        info.appendChild(title);
        // display storage entries if present
        if (s.storage && s.storage.length) {
          const storWrap = document.createElement('div'); storWrap.style.marginTop='6px';

            // compute category breakdown (capacity contribution considers slots for auto entries)
            const categoryMap = {};
            s.storage.forEach(st => {
              try{
            ...
            // render breakdown (ordered categories)
            ...
            if (parts.length === 0){
              breakdownEl.textContent = 'Breakdown: none';
            } else {
              breakdownEl.textContent = 'Breakdown: ' + parts.join(' • ');
            }
            storWrap.appendChild(breakdownEl);

            // Group auto storages by (capacity, slots) so identical groups render their slot lines once
            ...
            if (autos.length > 0){
              // build groups keyed by "cap_slots"
              const groups = {};
              autos.forEach(st => {
              ...
              // render each group once: show repeated slot lines for group's slot count
              Object.keys(groups).forEach(k => {
                const g = groups[k];
                if (g.slots && g.slots > 0){
                  for (let i=1;i<=g.slots;i++){
                    const div = document.createElement('div'); div.style.fontSize='12px'; div.style.color='#444';
                    div.textContent = 'auto — ' + g.cap + ' — slot ' + i;
                    storWrap.appendChild(div);
                  }
                } else {
                  // no slots -> single line
                  const div = document.createElement('div'); div.style.fontSize='12px'; div.style.color='#444';
                  div.textContent = 'auto — ' + g.cap;
                  storWrap.appendChild(div);
                }
              });
            }

            // render non-auto storages as before
            nonAutos.forEach(st => {
              const capNum = (st.capacity !== null && st.capacity !== undefined) ? Number(st.capacity) : NaN;
              const capText = (isFinite(capNum) && capNum > 0) ? (' — ' + st.capacity + (st.unit ? (' ' + st.unit) : '')) : '';
              const slotsNum = (st.slots !== undefined && st.slots !== null) ? Number(st.slots) : null;
              const repeat = (String(st.storage_type||'').toLowerCase() === 'auto' && slotsNum && slotsNum>0) ? slotsNum : 1;
              for (let r=0; r<repeat; r++){
                const text = (st.storage_type || st.kind || '') + capText;
                const div = document.createElement('div'); div.style.fontSize='12px'; div.style.color='#444';
                div.textContent = repeat>1 ? (text + ' — slot ' + (r+1)) : text;
                storWrap.appendChild(div);
              }
            });
        ...
        const edit = document.createElement('button'); edit.textContent='Edit'; edit.className='btn ghost'; edit.addEventListener('click', ()=>{ openEditDialog(s); });
          ...
          const manage = document.createElement('button'); manage.textContent='Manage Stock'; manage.className='btn positive'; manage.style.marginLeft='8px'; manage.addEventListener('click', ()=>{
            const selEl = document.getElementById('select-facility');
            const q = new URLSearchParams(); q.set('facility_id', s.facility_id || (selEl?selEl.value:'')); q.set('structure_id', s.id); q.set('structure_name', s.structure_name);
            window.open('stockpiles.php?'+q.toString(), '_blank');
          });
          btns.appendChild(manage);
          // structure-level Edit Slots button: updates all 'auto' storages for this structure
          const editStructureSlots = document.createElement('button'); editStructureSlots.textContent = 'Edit Slots'; editStructureSlots.className='btn ghost'; editStructureSlots.style.marginLeft='6px'; editStructureSlots.addEventListener('click', async ()=>{
            const currentSlots = (s.storage && s.storage.length && s.storage[0].slots) ? Number(s.storage[0].slots) : 15;
            const v = prompt('Set slots for auto storages in this structure (integer)', String(currentSlots)); if (v === null) return; const nv = parseInt(v) || 0;
            const rr = await post('update_structure_slots', { structure_id: s.id, slots: nv }); if (!rr.ok) return alert(rr.error || 'failed'); loadAssignedStructures();
          });
          btns.appendChild(editStructureSlots);
          // storage management buttons
          const addSilo = document.createElement('button'); addSilo.textContent='Add Liquid Silo (5000L)'; addSilo.className='btn ghost'; addSilo.addEventListener('click', async ()=>{
            const rr = await post('add_storage_entry',{structure_id: s.id, kind: 'Liquid Silo', capacity: 5000, unit: 'L'});
            if (rr.ok) loadAssignedStructures(); else alert(rr.error||'failed');
          });
          const addLiqTransfer = document.createElement('button'); addLiqTransfer.textContent='Add Liquid Transfer Station (500 items)'; addLiqTransfer.className='btn ghost'; addLiqTransfer.addEventListener('click', async ()=>{
            // ask which liquid
            const choice = prompt('Enter liquid type (Heavy oil, Petrol, E-oil, Water, Diesel)'); if (!choice) return;
            const volumes = { 'Heavy oil':30, 'Petrol':50, 'E-oil':30, 'Water':50, 'Diesel':100 };
            const pv = volumes[choice] || 0;
            const rr = await post('add_storage_entry',{structure_id: s.id, kind: 'Liquid Transfer Station', capacity: 500, unit: 'items', allowed_item: choice, per_item_volume: pv});
            if (rr.ok) loadAssignedStructures(); else alert(rr.error||'failed');
          });
          const addResTransfer = document.createElement('button'); addResTransfer.textContent='Add Resource Transfer Station (15000)'; addResTransfer.className='btn ghost'; addResTransfer.addEventListener('click', async ()=>{
            const rr = await post('add_storage_entry',{structure_id: s.id, kind: 'Resource Transfer Station', capacity: 15000, unit: 'units'});
            if (rr.ok) loadAssignedStructures(); else alert(rr.error||'failed');
          });
          const addMatTransfer = document.createElement('button'); addMatTransfer.textContent='Add Material Transfer Station (1000 items)'; addMatTransfer.className='btn ghost'; addMatTransfer.addEventListener('click', async ()=>{
            const rr = await post('add_storage_entry',{structure_id: s.id, kind: 'Material Transfer Station', capacity: 1000, unit: 'items'});
            if (rr.ok) loadAssignedStructures(); else alert(rr.error||'failed');
          });
          const addSepLiquid = document.createElement('button'); addSepLiquid.textContent='Add Separate Liquid'; addSepLiquid.className='btn ghost'; addSepLiquid.style.marginLeft='6px'; addSepLiquid.addEventListener('click', async ()=>{
            // ask which liquid and capacity (default to page-wide liquid separate capacity if present)
            const defaultCapEl = document.getElementById('liquid-separate-capacity');
            const defaultCap = defaultCapEl? (parseInt(defaultCapEl.value)||1000) : 1000;
            const choice = prompt('Enter liquid type (e.g. Water (L) or Heavy Oil (L))'); if (!choice) return;
            const capStr = prompt('Capacity in liters', String(defaultCap)); if (capStr===null) return;
            const cap = parseInt(capStr) || defaultCap;
            const rr = await post('add_storage_entry',{structure_id: s.id, kind: 'Separate Liquid', storage_type: 'Separate Liquid', capacity: cap, unit: 'L', allowed_item: choice});
            if (rr.ok) loadAssignedStructures(); else alert(rr.error||'failed');
          });
    ...
    if (addStructBtn) addStructBtn.addEventListener('click', async ()=>{
      ...
      // gather liquid capacities if visible (element may be absent)
    ...
    // wire up quick-add buttons (guard elements may be missing)
    ...
    async function loadUsers(){
      ...
      const roles = ['Member','Officer','Head of Facility','Head of MPF','Admin'];
        ...
        const save = document.createElement('button'); save.textContent='Save'; save.className='btn primary'; save.addEventListener('click', async ()=>{
          const newRole = sel.value;
          const res = await post('update_user_role', { id: u.id, role: newRole });
          if (!res.ok) return alert('Failed: '+(res.error||'unknown'));
          save.textContent = 'Saved'; setTimeout(()=>save.textContent='Save', 1000);
        });
        // disable controls for non-Officers
    ...
    // Tabs: simple client-side switching
    (function(){
